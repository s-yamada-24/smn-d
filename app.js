...
function renderConnectionProperties(basicTab, elementsTab, portsTab) {
    ...
    basicTab.innerHTML = `
        <table class="properties-table">
            <tr>
                <th>接続元</th>
                <td>${fromNode ? fromNode.label : '不明'}</td>
            </tr>
            <tr>
                <th>接続先</th>
                <td>${toNode ? toNode.label : '不明'}</td>
            </tr>
        </table>
        <div style="margin-top: 16px;">
            <button class="btn btn-secondary" id="reconnectBtn" style="width: 100%; margin-bottom: 8px;">🔄 接続を繋ぎ変える</button>
            <button class="btn btn-secondary" id="deleteConnectionBtn" style="width: 100%; background: #da3633; border-color: #da3633;">🗑️ 接続を削除</button>
        </div>
    `;
...
function renderGroupProperties(basicTab, elementsTab, portsTab) {
    ...
    if (group.children.length === 0) {
        ...
        let html = `
            <div class="section-header">
                <span class="section-title">子要素一覧</span>
            </div>
    ...
    node.elements.forEach((el, index) => {
        html += `
            <tr>
                ...
                <td class="action-cell">
                    <button class="btn-icon" onclick="deleteElement(${index})">🗑️</button>
                </td>
            </tr>
        `;
    });
    ...
    ports.forEach((port, index) => {
        html += `
            <tr>
                ...
                <td class="action-cell">
                    <button class="btn-icon" onclick="deletePort('${side}', ${index})">🗑️</button>
                </td>
            </tr>
        `;
    });
